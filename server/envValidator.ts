...
export function validateEnvironment(): ValidationResult {
  ...
  if (isProduction) {
    if (!process.env.ENCRYPTION_KEY) {
      errors.push(
        'ENCRYPTION_KEY is required in production for encrypting AWS SES credentials. ' +
    ...
    if (!process.env.TRACKING_SECRET) {
      errors.push(
        'TRACKING_SECRET is required in production for validating tracking tokens. ' +
    ...
    if (!process.env.ENCRYPTION_KEY) {
      warnings.push(
        'ENCRYPTION_KEY not set. A temporary key will be generated for development. ' +
    ...
    if (!process.env.TRACKING_SECRET) {
      warnings.push(
        'TRACKING_SECRET not set. A random secret will be generated for development. ' +
